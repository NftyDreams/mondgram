...
class Mondgram {
    ...
    constructor(id, options, mondgramData) {
        this.id = id;
        this.size = 128;
        this.options = options || {};
        this.scale = this.options.scale || 1;
        this.granularity = this.options.granularity || 5;

        this.colors = this.options.colors || ['white', 'red', 'yellow', 'blue'];
        this.thickness = 2;
        this.emoji = null;
        if (mondgramData) {
            this.randoms = Mondgram._toRandoms(mondgramData);
            this.emoji = mondgramData.split('&')[1];
            this.play = true;
        } else {
            this.randoms = [];
            this.play = false;
    ...
    _init() {
        ...
        rect._split(xPad, yPad, 0, self.granularity, ctx);
        ctx.stroke();
    ...
    static _toRandoms(mondgramData) {
        let rndData = mondgramData.split('&')[0];
    ...
    mondgram() {
        if (this.randoms.length > 0) {
            ...
            return window.btoa(binary).replace('==', '') + '&' + Mondgram._getEmoji();
        } else {
